...
x_smooth = 0:0.005:1;
plot(x_smooth, eigf1(x_smooth), '-k');
plot(x_smooth, eigf2(x_smooth), '-.k');
plot(x_smooth, eigf3(x_smooth), '--k');
...
plot(x_vec, [0; V_fem(:,1) / V_fem(max_ind,1)], 'ok');
[~, max_ind] = max(abs(V_fem(:,1)));
plot(x_vec, [0; -1*V_fem(:,2) / V_fem(max_ind,2)], 'ok');
[~, max_ind] = max(abs(V_fem(:,1)));
